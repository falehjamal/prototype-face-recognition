...
-- Contoh data tenant
-- INSERT INTO tenants (name, db_host, db_port, db_name, db_user, db_pass, status) VALUES
-- ('Sekolah ABC', '127.0.0.1', 3306, 'sekolah_abc', 'root', '', 'active'),
-- ('Sekolah XYZ', '127.0.0.1', 3306, 'sekolah_xyz', 'root', '', 'active');


-- =========================================
-- TENANT DATABASE: (jalankan di masing-masing database tenant)
-- =========================================

-- Tabel users (contoh: user_1 untuk tenant_id=1)
-- Tabel ini diasumsikan sudah ada di setiap tenant database
-- CREATE TABLE IF NOT EXISTS `user_{tenant_id}` (
--     `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
-- Tabel enrollment (contoh: enrollment_1 untuk tenant_id=1)
-- Ganti {tenant_id} dengan ID tenant yang sesuai
-- CREATE TABLE IF NOT EXISTS `enrollment_{tenant_id}` (
--     `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
--     `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
--     
--     CONSTRAINT `fk_enrollment_{tenant_id}_user` FOREIGN KEY (`user_id`) 
--         REFERENCES `user_{tenant_id}`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
--     
--     INDEX `idx_enrollment_user_id` (`user_id`),
--     INDEX `idx_enrollment_label` (`label`),
